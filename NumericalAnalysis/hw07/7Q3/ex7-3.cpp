...
int main(){
	...
	char dummy[256];//空読み用
	int ret;
    double k[128],x[128],y[128],a,az,b,bz,s,sz;//配列の要素の数は多めに
	int i,j,nz;
	double fi;
	long t0,t1;//桁の多い整数
	double tz; 
	...
	fp = fopen("ex-07-3.txt","r"); //データファイル」を読み取り専用rで開く。
    fgets(dummy,256,fp); //1行目を空読みする。
	j=1;
	while( ( ret = fscanf( fp,"%l\t%lf\t%lf",&k[j],&x[j],&y[j]) ) != EOF ){  //データ x[j],y[j] を読む。制御コード \tはTab。ＮＯＴEOFである限り繰り返す。
	                                       //2行目から読み取り、一行目は空読み済み
		nz=j; //nz:データ数
		j=j+1;
	}
